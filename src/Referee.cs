	...
	public class Referee {
		...
		public Properties init(Properties _params) {
			...
			foreach (Unit unit in Const.game.allUnits) {
				if (unit is Tower) gameManager.getActivePlayers().get(unit.team).tower = (Tower)unit;
			}
		...
		public void gameTurn(int turn) {
			...
			foreach (Player player in gameManager.getPlayers()) {
				...
				player.sendInputLine($"{(turn >= Const.HEROCOUNT) ? player.heroes.Where(h => !h.isDead).Count() : (turn - Const.HEROCOUNT)}");
		...
		S	}
			catch (Exception e)
			{
				...
				if (e is TimeoutException)
					msg = " timeout";
				else if (e is IndexOutOfBoundsException || e is IllegalArgumentException)
				{
					gameManager.addToGameSummary(player.getNicknameToken() + " supplied an invalid hero name: " + output);
					msg = " supplied an invalid hero name: " + output;
				} else
